...
for epoch in range(num_epochs):
    ...
    for batch_idx, (images, _) in enumerate(train_loader):
        ...
        for img_idx, img in enumerate(corrupted_images):
            ...
            x = np.random.randint(-d, D)  # x can now be negative, but must be within the range of [-d, D)
            y = np.random.randint(-h, H)  # y can now be negative, but must be within the range of [-h, H)
            z = np.random.randint(-w, W)
            x_end = min(D, x + d)
            y_end = min(H, y + h)
            z_end = min(W, z + w)
            x = max(0, x)
            y = max(0, y)
            z = max(0, z)     
            img[:, x:x_end, y:y_end, z:z_end] = 0
            print(f"Corrupted region for image {img_idx}: x={x}, x_end={x_end}, y={y}, y_end={y_end}, z={z}, z_end={z_end}")
