...
def extract_features_with_pretrained_ddpm(
    ...
    first_image_vector_printed = False
        ...
        for batch_idx, (images, labels) in enumerate(loader):
            ...
            for b in range(B):
                    ...
                    for i_ in range(2):
                        ...
                        for j_ in range(2):
                            ...
                            for k_ in range(2):
                                ...
                                orig_feats, feature_names = first_order_and_shape_features(
                                    original_patch,
                                ...
                                excluded_orig_feats = []
                                excluded_recon_feats = []

                                for key, value in orig_feats.items():
                                    if isinstance(value, torch.Tensor) and torch.isnan(value).any():
                                        print(f"[ERROR] NaN in orig_feats at batch {batch_idx}, key={key}")
                                        excluded_orig_feats.append(key)
                                
                                if excluded_orig_feats:
                                    print(f"[INFO] Total excluded original features: {len(excluded_orig_feats)}")
                                    print(f"[INFO] Excluded original features: {excluded_orig_feats}")
                                ...
                                for key, value in recon_feats.items():
                                    if isinstance(value, torch.Tensor) and torch.isnan(value).any():
                                        ...
                                        excluded_recon_feats.append(key)

                                if excluded_recon_feats:
                                    print(f"[INFO] Total excluded reconstructed features: {len(excluded_recon_feats)}")
                                    print(f"[INFO] Excluded reconstructed features: {excluded_recon_feats}")
                ...
                if len(sample_feats_all_views) > 0:
                    ...
                    if not first_image_vector_printed:
                        print("[INFO] Feature vector (1824 features) for the first image:")
                        print(sample_feats_tensor.tolist())  # Convert tensor to list for readable output
                        first_image_vector_printed = True  # Ensure we only print once
    ...
    if len(all_features) > 0:
        ...
        return all_features, all_labels, valid_indices, full_feature_names
    else:
...
def train_cnn_with_masking(
    ...
    for _, _, labels in train_loader:
        train_labels_full.append(labels[:, 0].unsqueeze(1))  # Extract only ACL labels
    ...
    pos_weight = np.clip(pos_weight, 0.4, 2.75)  # Limit range

    pos_weight_tensor = torch.tensor(pos_weight, dtype=torch.float, device=device)
    print(f"[DEBUG] abnormal pos_weight = {pos_weight:.3f}")
    ...
    for epoch in range(num_epochs):
        ...
        for images_3d, feats_1824, labels in train_loader:
            ...
            labels = labels.to(device)[:, 0].unsqueeze(1)  # Extract ACL labels
        ...
        #print(f"[DEBUG] Global Mask (First 2): {model.cnn_model.global_mask[:8].detach().cpu().numpy()}")
        ...
        with torch.no_grad():
            for images_3d, feats_1824, labels in val_loader:
                ...
                labels = labels.to(device)[:, 0].unsqueeze(1)
    ...
    with torch.no_grad():
        for images_3d, feats_1824, labels in val_loader:
            ...
            all_labels_val.append(labels[:, 0].unsqueeze(1).cpu())
...
if __name__ == "__main__":
    ...
    features_path = os.path.join(model_save_path, "features_1824_abn_clinical.pt")
    labels_path = os.path.join(model_save_path, "labels_3_abn_clinical.pt")
    norm_stats_path = os.path.join(model_save_path, "normalization_stats_abn_clinical.json")

    feature_importance_path = os.path.join(model_save_path, "feature_importance_abn_clinical.json")
    ...
    labels_abn = labels_3[:, 0].unsqueeze(1) 

    train_indices, val_indices = train_test_split(
        np.arange(len(labels_abn)),
        test_size=0.27,  # 25% for validation
        stratify=labels_abn.cpu().numpy(),
        random_state=42
    )
