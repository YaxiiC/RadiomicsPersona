...
def compute_metrics(probabilities, labels):
    ...
    # Print results
    print(f"abnormal Metrics: Acc={accuracy:.3f}, Sensitivity={sensitivity:.3f}, "
...
def find_best_thresholds_youden(probabilities, labels, min_threshold=0.1):
    ...
    print(f"Best threshold for abnormal = {best_thr:.2f}, Youden's J = {best_j:.3f}")
...
def evaluate_model(model, loader, device, threshold=0.53, save_path="patient_feature_importance.json"):
    model.eval()
    all_probs, all_labels = [], []
    
    # We will store the feature importances for the last two samples of the last batch
    patient_feature_importances = []
    last_batch_feature_importance = None
    ...
    with torch.no_grad():
        for batch_idx, (images_3d, feats_1824, labels) in enumerate(loader):
            ...
            # Extract the importance vector from the CNN model
            feature_importance = model.cnn_model(images_3d)  # [B, 1824]
            
            # Save the feature_importance from this batch. We only actually write it to JSON after the loop ends.
            last_batch_feature_importance = feature_importance
            
            # Apply hard selection: set importance < 0.40 to 0, >= 0.40 to 1
            feature_importance = torch.where(feature_importance < 0.40, 
                                             torch.zeros_like(feature_importance), 
                                             feature_importance)
            feature_importance = torch.where(feature_importance >= 0.40, 
                                             torch.ones_like(feature_importance), 
                                             feature_importance)

            nonzero_counts = (feature_importance > 0).sum(dim=1)  # count non-zero per sample
            print(f"Batch {batch_idx}: non-zero importance counts per sample: {nonzero_counts}")

            masked_feats = feats_1824 * feature_importance
    ...
    # Once all batches have been processed, we save the feature importance for
    # the last two samples in the final batch (if it exists).
    if last_batch_feature_importance is not None:
        # Determine how many samples were in the last batch
        bsize = last_batch_feature_importance.shape[0]
        
        # We'll grab the last two samples. If there's only 1 sample, it will handle that as well.
        start_idx = max(0, bsize - 2)
        for i in range(start_idx, bsize):
            importance_vector = last_batch_feature_importance[i]  # shape [1824]
            importance_values = importance_vector.cpu().numpy().tolist()
            
            # Get sorted indices in descending order
            sorted_indices = torch.argsort(importance_vector, descending=True).cpu().numpy().tolist()
            
            # Create (index, importance) pairs in descending order
            importance_pairs = [
                {"index": idx, "importance": importance_values[idx]} 
                for idx in sorted_indices
            ]
            
            # Generate a pseudo ID for this sample. You can customize as needed.
            patient_id = f"last_batch_sample_{i}"
            
            patient_data = {
                "patient_id": patient_id,
                "importance_pairs": importance_pairs
            }
            patient_feature_importances.append(patient_data)

        # Write out the JSON with importance info for these last two samples
        with open(save_path, 'w') as f:
            json.dump(patient_feature_importances, f, indent=4)
        print(f"[INFO] Feature importance for the last two samples in the final batch saved to {save_path}")

    # Concatenate probabilities and labels for overall metrics
    all_probs = torch.cat(all_probs, dim=0)
    all_labels = torch.cat(all_labels, dim=0)

    preds = (all_probs >= threshold).float()

    # Simple example: using compute_metrics with preds & labels
        ...
        for fold, (train_idx, val_idx) in enumerate(skf.split(np.zeros(len(labels_np)), labels_np.argmax(axis=1))):
            ...
            for epoch in range(5):  # Train for 5 epochs per fold
                for images_3d, feats_1824, labels_3 in train_loader:
                    ...
                    loss = nn.BCEWithLogitsLoss()(logits, labels_3[:, 0].unsqueeze(1).float())  # Extract ACL label
...
if __name__ == "__main__":
    ...
    thresholds_norm_path = os.path.join(model_save_path, "thresholds_and_normalization_softmax_2*2_abn_clinical.json")
    ...
    metrics_dict = evaluate_model(
        ...
        loader=test_loader,
        device=device,
        threshold=threshold if threshold is not None else 0.53,
        save_path=os.path.join(model_save_path, "patient_feature_importance.json")
    )

    with open(output_log_file, "w") as f:
        f.write("Evaluation Results (Abnormal):\n")
