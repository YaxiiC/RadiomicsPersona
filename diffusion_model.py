...
class DDPM3D(nn.Module):
    ...
    def forward(self, x_noisy, t):
        noise_pred = self.unet(x_noisy)
        return noise_pred

    def add_noise(self, x0, t):
        alphas_cumprod_t = self.alphas_cumprod[t].view(-1, 1, 1, 1, 1)
        sqrt_alphas_cumprod_t = torch.sqrt(alphas_cumprod_t)
        one_minus_alphas_cumprod_t = 1.0 - alphas_cumprod_t
        eps = torch.randn_like(x0)
        x_t = sqrt_alphas_cumprod_t * x0 + torch.sqrt(one_minus_alphas_cumprod_t) * eps
        return x_t, eps
