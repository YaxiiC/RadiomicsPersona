...
def evaluate_model(model, loader, device, threshold=0.53, save_path="patient_feature_importance.json"):
    ...
    with torch.no_grad():
        for batch_idx, (images_3d, feats_1824, labels) in enumerate(loader):
            ...
            labels = labels[:, 2].unsqueeze(1).to(device)
        ...
        for fold, (train_idx, val_idx) in enumerate(skf.split(np.zeros(len(labels_np)), labels_np.argmax(axis=1))):
            ...
            for epoch in range(5):  # Train for 5 epochs per fold
                for images_3d, feats_1824, labels_3 in train_loader:
                    ...
                    loss = nn.BCEWithLogitsLoss()(logits, labels_3[:, 2].unsqueeze(1).float())  # Extract ACL label
...
if __name__ == "__main__":
    ...
    with open(output_log_file, "w") as f:
        f.write("Evaluation Results (Meniscus):\n")
